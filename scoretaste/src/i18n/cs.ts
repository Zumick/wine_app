/**
 * Jednojazyčný slovník (cs). Klíče odpovídají `t("segment.key")`.
 * Později lze přidat další jazyky paralelně k tomuto souboru.
 */
export const cs = {
  common: {
    loading: "Načítání akce…",
    error: "Chyba",
    hintRetry:
      "Zkontrolujte odkaz nebo zkuste stránku načíst znovu.",
    hintCheckLink: "Zkontrolujte odkaz.",
    backToWineryList: "← Seznam vinařství",
  },
  guide: {
    title: "ScoreTaste",
    skeletonNote: "Skeleton — base path /guide/",
  },
  visitor: {
    infoAria: "Informace o ScoreTaste",
    navAria: "Hlavní navigace",
    navWineries: "Vinařství",
    navMyWines: "Moje vína",
    filterPlaceholder: "Filtrovat vinařství…",
    filterAria: "Filtrovat seznam vinařství",
    filterClearAria: "Zrušit filtr",
    infoTitle: "O aplikaci",
    infoBody: "Průvodce degustací",
    infoFooterMadeBy: "made by",
    modalCloseAria: "Zavřít",
    modalOk: "Rozumím",
    wineCountLabel: "vín",
    filterNoResults: "Žádné vinařství neodpovídá filtru.",
    epochSyncNote: "Stav akce byl aktualizován.",
    returnHintAria: "Jak se vrátit do průvodce",
    returnModalTitle: "Jak se vrátit do průvodce",
    returnUrlLabel: "Adresa akce (uložte si ji nebo zkopírujte):",
    copyLink: "Kopírovat odkaz",
  },
  winery: {
    title: "Vinařství",
    viewMap: "Mapa",
    viewList: "Seznam",
    mapSwitchAria: "Přepnout mezi seznamem a mapou",
    mapImageAlt: "Mapa areálu akce",
    mapMarkerAria: "Otevřít detail vinařství",
    detailToggleAria: "Zobrazit nebo skrýt poznámku a web vinařství",
    visitedToggleAriaOn: "Označit sklep jako navštívený",
    visitedToggleAriaOff: "Zrušit označení navštíveného sklepu",
    addWine: "Přidat víno",
    cellarWord: "Sklep",
    noneInEvent: "Žádná vinařství v této akci.",
    notFoundStrong: "Vinařství nenalezeno.",
    winesHeading: "Vína",
    noWinesHere: "Žádná vína u tohoto vinařství.",
    colorWhite: "Bílé",
    colorRose: "Růžové",
    colorRed: "Červené",
    colorOrange: "Oranžové",
    colorWhiteWines: "Bílé vína",
    colorRoseWines: "Růžová vína",
    colorRedWines: "Červená vína",
    colorOrangeWines: "Oranžová vína",
  },
  wine: {
    starAriaNone: "Hvězda: bez označení. Klepnutím uložit mezi oblíbená.",
    starAriaFavorite: "Hvězda: oblíbené. Klepnutím zvýraznit jako TOP.",
    starAriaTop: "Hvězda: TOP. Klepnutím vrátit na oblíbené.",
  },
  myWines: {
    title: "Moje vína",
    empty:
      "Zatím nemáte žádná označená vína — u vinařství klepněte na hvězdu vlevo u řádku vína.",
    emptySavedCta: "Přejít na seznam vinařství",
    showGrouped: "Podle vinařství",
    showFlat: "Zobrazit seznam",
    shareAria: "Sdílet můj výběr",
    shareMySelection: "Sdílet můj výběr",
    copiedToast: "Text zkopírován",
    savedShareToast: "Uloženo – sem se můžete kdykoli vrátit",
    restoredSelectionToast: "Načten váš uložený výběr",
    shareSaveError: "Nepodařilo se uložit výběr. Zkuste to znovu.",
    removalCountdown: "Odstranění za {seconds} s",
    cancelRemoval: "Zrušit",
    cancelRemovalAria:
      "Probíhá odpočet odstranění. Klepnutím zrušit odstraňování.",
    topLongPressAria:
      "Hvězda: uložené víno. Klepnutím přepínáte oblíbené/TOP. Podržením spustit odstranění.",
    removedToast: "Víno odstraněno",
    undo: "Zpět",
  },
  errors: {
    notFound: "Akce nenalezena.",
    invalidEvent: "Neplatná data akce.",
    missingEventId: "Chybí identifikátor akce.",
    loadFailed: "Akci se nepodařilo načíst.",
    generic: "Nelze načíst akci.",
    missingUrlParams: "Chybí parametry v URL.",
  },
  contributor: {
    title: "Příspěvek vinařství",
    eventIdLabel: "eventId:",
    wineryIdLabel: "wineryId:",
    missing: "(chybí)",
  },
} as const;
